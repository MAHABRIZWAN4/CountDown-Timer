...
let response = await inquirer.prompt([
  {
    name: "userInput",
    type: "number",
    message: chalk.bgBlue.bold.italic(
...
console.log(
  gradientText(`\t\tDear ! ${name.userName} CountDown Timer starts Now`)
);
console.log(
  gradientText("\t\t**********************************************\n")
);
...
function startTime(val: number) {
  ...
  setInterval(() => {
    ...
    if (timeDiff <= 0) {
      console.log(
        chalk.bgRedBright.underline.bold.italic(
          `Dear ${name.userName} Timer has expired!!!!`
        )
      );
      process.exit();
    }
